...
Customer = {
  ...
  addEventListeners: function() {
    var _self = this;
    $(document).on("click", "a.onAgregarDireccion", function(e) {
      _self.agregarDireccion(e);
    });
    $(document).on("click", "a.onLlamada", function(e) {
      _self.llamada(e);
    });
    $(document).on("click", "a.onEvaluar", function(e) {
      _self.evaluar(e);
    });
    $(document).on("click", "a.onClickVotoCiudad", function() {
      _self.votoCiudad();
    })
    $(document).on("click", "a.onClickEnviar", function(e) {
      _self.enviarChat(e);
    });
    $(document).on("click", "a.onActualizarDireccion", function(e) {
      _self.actualizarDireccion(e);
    });
    $(document).on("click", "a.onBorrarDireccion", function(e) {
      _self.borrarDireccion(e);
    });
    $(document).on("click", "a.onAgregarFavorito", function(e) {
      _self.agregarFavorito(e);
    });
    $(document).on("click", "a.onBorrarFavorito", function(e) {
      _self.borrarFavorito(e);
    });
    $(document).on("click", "a.onUpdateProfile", function(e) {
      _self.updateProfile(e);
    });
    $(document).on("click", "a.onClickCotizacion", function(e) {
      _self.cotizacion(e);
    });
    $(document).on("click", "a.onClickCotizar", function(e) {
      _self.cotizar(e);
    });
    $(document).on("click", "div.onClickGetCompartir", function(e) {
      _self.getCompartir(e);
    });
    $(document).on("click", "a.onClickCompartir", function(e) {
      _self.compartir(e)
    });
  },
  /* PROFILE */
  ...
  compartir: function(e) {
    ...
    var direcciones = [];
    var buffer = '<div class="col s10 offset-s2 color-azul div-msj"><div class="caja-msj txt-blanco color-azul">';
    ...
    if (data.correo) {
      buffer += '<div class="valign-wrapper"><i class="material-icons">email</i>' + $('.correo').html() + "<br></div>";
    }
    $('#pos').is(':checked') ? buffer+= '<a class="ignore" href="negocio_usuario.html?usuario='+data.id_usuario+'&negocio='+data.id_negocio+'&requerimiento='+data.id_cotizacion+'">\
    <img class="ubicacion" src="https://app.homepro.mx/admin/assets/images/geopoint.png"><br>Ubicación</a><br>' : true;
    if (data.movil) {
      // :(
      var telefono = $('.movil').html();
      telefono = telefono.replace('Teléfono', '');
      telefono = telefono.replace('(','');
      telefono = telefono.replace(')','');
      telefono = telefono.replace(' ','');
      buffer += '<div class="valign-wrapper"><i class="material-icons">smartphone</i><a href="tel:' + telefono + '"">' + $('.movil').html() + '</a></div>';
    }
    $('#message').val(buffer + '</div></div>');
    $('.onClickEnviar').click();
    $('#modal1').modal('close');
    if ($('#pos').is(':checked')) {
      ...
      swal('Para compartir tu ubicación es necesario que actives tu geolocalización');
      Service.execute("_ctrl/ctrl.service.php", {
        exec: "compartir",
        data: data
      }, function(r) {
  ...
  cotizacion: function() {
    ...
    if (!localStorage.getItem('cotizacion')) {
      // console.log('tu cotizacion esta siendo procesada');
      return;
    }
            ...
            function(r) {
              if (r.status == 202) {
                swal({
                  ...
                  text: "Su cotización se ha enviado a los proveedores, pronto recibirá respuesta.",
                  type: "success",
                  confirmButtonText: "Aceptar",
                  confirmButtonColor: "#2C8BEB"
                }, function(isConfirm) {
  ...
  cotizar: function() {
    ...
    var id_servicio = getUrlVars()["servicio"];
    info.id_servicio = id_servicio;
    info.id_usuario = getUid();
    console.log(info);

    /**
     * Checa que radiobox esta seleccionado para calcular la fecha, si no es esta seleccionado hoy o mañana
     * toma la fecha que el usuario ingresó, despues lo almacena en ls y redirecciona a cotizacion.html
     *
     */
    ...
    if (flag == 0) {
      ...
      swal({
        ...
        if (isConfirm) {
          location.href = "cotizacion.html?servicio=" + id_servicio;
        }
      });
    } else if (flag == 2 && info.cotizacion == '') {
      swal({
        title: "Error!",
  ...
  getChat: function() {
    ...
    var header = '<a href="negocio.html?negocio=' + info.id_negocio + '">\
          ...
          if (data.length > 0) {
            ...
            data.forEach(function(element) {
              ...
              if (dif == 0 && mes == 0) {
                ...
                fecha = (dia.getDate()) + '/' + (meses[dia.getMonth()]) + '/' + dia.getFullYear();
              }
              if (dia.getMinutes() < 10) {
                minuto = '0' + dia.getMinutes();
              } else {
                minuto = dia.getMinutes();
              }
              // hora = dia.getHours() + ':' + minuto;
              hora = element.created_at.slice(11, 16);
              if (fecha_anterior == dia.getDate() + '/' + dia.getMonth()) {
                fecha = '';
              }
              ...
              if (element.tipo_usuario == 0) {
                if (element.mensaje.indexOf('materialboxed') > -1) {
                  buffer += '<div class="'+element.id+'">' + element.mensaje + '<div class="col s10 offset-s2 right-align txt-gris-claro hora"><p style="font-size: 20px;margin-bottom:0px;" onclick="Customer.deleteMsj('+element.id+')"><i class="material-icons txt-azul">clear</i></p>' + hora + '</div></div>';
                } else if(element.mensaje.indexOf('div-msj') > -1){
                  buffer += element.mensaje;
                }else{
                ...
                buffer += '<div class="col s10 div-msj">\
                                    ...
                                    <span class="txt-gris-claro center-align hora">' + hora + ' </span>\
                            </div>';
              }
              dif = hoy.getDate();
              mes = hoy.getMonth();
            ...
            $('a.ignore').attr('href', null);
          }
        } else if (r.status == 0) {
          $('.chat').html('<div class="txt-gris-claro margint-20 center-align vacio">Aún no tienes ningún mensaje.</div>');
    ...
    if (flag == 0) {
      DAO.execute("_ctrl/ctrl.service.php", {
        ...
        function(r) {
          ...
          if (fecha_anterior == fecha.getDate() + '/' + fecha.getMonth()) {
            fecha_imprimir = ''
          } else {
            fecha_imprimir = 'Hoy';
          }
  ...
  agregarDireccion: function() {
    ...
    if (!info.estado || info.estado.length < 3) {
      $('.estado .select-wrapper .select-dropdown').addClass('invalid');
      flag = 1;
    }
    ...
    if (!info.id_ciudad) {
      $('.ciudad .select-wrapper .select-dropdown').addClass('invalid');
    }
          ...
          if (direcciones) {
            direcciones.forEach(function(element) {
              buffer += '<div class="row valign-wrapper">\
                        ...
                        <p>\
                        <a href="editar_direccion.html?direccion=' + element.id + '"><i class="material-icons">edit</i></a>\
                        <br>\
                        </p>\
                        </div>\
                        </div><hr>';
            ...
            $('.lista-direcciones').html('<div class="center-align txt-gris-claro">No tienes ninguna dirección.</div>');
          }
        } else if (r.status == 404) {
          swal({
            title: "",
  ...
  cotizacionNegocios: function(id_zona, id_cotizacion) {
    var info = new Object();
    info.id_servicio = getUrlVars()['servicio'];
    info.id_usuario = getUid();
    if (id_zona) {
    ...
    Service.execute("_ctrl/ctrl.service.php", {
      ...
      function(r) {
        if (r.status == 202) {
          ...
          if (id_cotizacion) {
            data.forEach(function(element) {
              ...
              buffer += '<li class="collection-item">\
                        ...
                        <br>\
                        <a href="#modal3" class="secondary-content" onClick="Customer.negocioDetalles(' + element.id + ')"><i class="material-icons">visibility</i></a>\
            ...
            data.forEach(function(element) {
              ...
              zona = zona.substring(0, zona.length - 2);
              buffer += '<li class="collection-item">\
                            <a href="negocio.html?negocio=' + element.id + '" class="">\
          ...
          $('.list').empty().html('<div class="txt-gris-claro center-align margint-20">No hay ningun negocio en esta zona</div>');
          ...
          if (id) {
            if (zonas) {
              ...
              $(document).on('change', '#zona', function() {
                Customer.cotizacionNegocios($('#zona').val(), id);
              })
              buffer = '';
            ...
            if (zonas) {
              ...
              $('#zona').html(buffer);
              $(document).on('change', '#zona', function() {
                Customer.cotizacionNegocios($('#zona').val());
              })
              buffer = '';
              $('select').material_select();
    ...
    Service.execute("_ctrl/ctrl.service.php", {
      ...
      function(r) {
        if (r.status == 202) {
          ...
          $('.descripcion').empty().html(data.negocio[0].informacion);
          if (data.testimonios) {
            ...
            $('#testimonios').empty().html('<div class="txt-gris-claro center-align">Este negocio aun no tiene ningún testimonio</div>');
          }
          ...
          if (data.negocio[0].movil && data.negocio[0].telefono) {
            $('.llamar').html('<a href="#modal2"><i class="material-icons">call</i><br>Llamar</a>');
            $('.celular').html('<a href="tel:' + data.negocio[0].movil + '" onClick="Customer.llamada(' + id_negocio + ')" class="valign"><i class="valign material-icons">smartphone</i>Celular</a>');
            $('.telefono').html('<a href="tel:' + data.negocio[0].telefono + '" onClick="Customer.llamada(' + id_negocio + ')" class="valign"><i class="valign material-icons">call</i>Teléfono</a>');
          } else {
            if (data.negocio[0].telefono) {
              $('.llamar').html('<a href="tel:' + data.negocio[0].telefono + '" onClick="Customer.llamada(' + id_negocio + ')"><i class="material-icons">call</i><br>Llamar</a>');
            } else {
              $('.llamar').html('<a href="tel:' + data.negocio[0].movil + '" onClick="Customer.llamada(' + id_negocio + ')"><i class="material-icons">smartphone</i><br>Llamar</a>');
            }
          }
        } else {
        ...
        if (r.status == 202) {
          ...
          if (negocios) {
            for (var i = 0; i < negocios.length; i++) {
              ...
              buffer += '<li class="collection-item llamada"><a class="modal-trigger" href="#modal3" onClick="Customer.negocioDetalles(' + negocios[i].id + ')">\
            ...
            $('.collection').empty().html('<div class="txt-gris-claro center-align">Aún no tienes ningún negocio como favorito</div>');
          }
        } else {
          $('.collection').empty().html('<div class="txt-gris-claro center-align">Aún no tienes ningún negocio como favorito</div>');
        }
    ...
    Service.execute("_ctrl/ctrl.service.php", {
      ...
      function(r) {
        if (r.status == 202) {
          ...
          if (llamadas && cotizaciones) {
            historial = cotizaciones.concat(llamadas);
          } else if (cotizaciones) {
            historial = cotizaciones;
            $('#llamada').html('Aún no tienes ninguna llamada');
            $('#llamada').hide();
            $('#todos').removeClass('center-align txt-gris-claro');
            $('#mensaje').removeClass('center-align txt-gris-claro');
          } else if (llamadas) {
            historial = llamadas;
            $('#mensaje').html('Aún no tienes ningún mensaje');
            $('#mensaje').hide();
            $('#todos').removeClass('center-align txt-gris-claro');
            $('#llamada').removeClass('center-align txt-gris-claro');
          } else {
  ...
  switch: function(e) {
    if (e == 1) {
      ...
      $('.llamada').fadeIn(1);
      if ($('#llamada').html() == 'Aún no tienes ninguna llamada') {
        $('#llamada').hide()
      }
      if ($('#mensaje').html() == 'Aún no tienes ningún mensaje') {
        $('#mensaje').hide()
      }
      // $('.search').val('');
      // $('.search').click();
    ...
    Service.execute("_ctrl/ctrl.service.php", {
      ...
      function(r) {
        if (r.status == 202) {
          ...
          var negocios = r.data.negocios;
          if (r.data.imagenes[0].imagen && r.data.imagenes[0].imagen != 'null') {
              var imagenes = r.data.imagenes[0].imagen;
              imagenes = JSON.parse(imagenes);
              var buffer = '';
              imagenes.forEach(function(element) {
                  buffer += '<div class="box materialboxed" style="background:url(admin/uploads/fotos/' + element + ') top center no-repeat; background-size:auto 100%;"></div>';
                  $('.grid').addClass('gridimg');
                  $('.grid').html(buffer);
              });
              $('.materialboxed').materialbox();
          } else {
              var imprimir = '<div class="col s12">No hay ninguna imagen para mostrar.</div>';
              $('.grid').html(imprimir);
          }
          for (var i = 0; i < mensajes.length; i++) {
            if (mensajes[i]) {
          ...
          var fecha_atn = new Date(fecha_atn);
          fecha_atn = (fecha_atn.getDate()+1) + '/' + (meses[fecha_atn.getMonth()]) + '/' + fecha_atn.getFullYear();
          $('.collection').html(buffer);
          $('#titulo').html(negocios[0].descripcion);
          $('.fecha-atn').append(fecha_atn);
        } else {
      ...
      Service.execute("_ctrl/ctrl.service.php", {
        ...
        function(r) {
          if (r.status == 202) {
            ...
            $('#testimonio').val('');
            $('#calificacion').val('');
            $('.onClickStar').removeClass('active');
            swal({
              title: "",
              text: "Gracias por su evaluación, una vez aprobada aparecerá en los testimoniales.",
              type: "success",
              confirmButtonText: "Aceptar",
              confirmButtonColor: "#2C8BEB"
            });
