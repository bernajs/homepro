...
class Ciudad extends Helper {
    ...
    public function db($key){
        switch($key){
            case "insert":
                $this->status = 1;
                $query = "INSERT INTO voto_ciudad (created_at,ciudad,status)
                VALUES (
                '".$this->created_at."',
                '".$this->ciudad."',
                '".$this->status."'
            ...
            case "update":
                ...
                WHERE id=".$this->id;
                break;
            case "delete": $query = "DELETE FROM voto_ciudad WHERE id=".$this->id;
                break;
    }
    $lid = false;
...
public function getCiudades($id = NULL){
    $query = 'SELECT * FROM voto_ciudad WHERE id > 0';
