...
Negocio = {
    ...
    addEventListeners: function() {
        ...
        $(document).on("click", "div.onClickEnviar", function(e) { _self.enviarChat(e); });
        ...
        Service.execute("../_ctrl/ctrl.negocio.php", {
            ...
            function(r) {
                if (r.status == 202) {
                    ...
                    if (data.ver_perfil == 0 && data.btn_llamar == 0 && data.cotizaciones == 0) {
                        $('#myChart').hide();
                        $('.chart').html('<div class="col s12 txt-gris-claro margint-20 center-align">Aún no tienes ninguna estadística.</div>');
                    }
                } else {
                    swal({
                        title: "",
    ...
    testimonio: function() {
        ...
        Service.execute("../_ctrl/ctrl.negocio.php", {
            ...
            if (data) {
                ...
                $('#testimonios').empty().html('<div class="margint-20 center-align">Aún no tienes ningún testimonio.</div>');
            }
        });
    },
    // Historial
    ...
    getChat: function() {
        ...
        <i class="material-icons txt-blanco icono">keyboard_arrow_right</i>\
                ...
                <div class="col s12 txt-blanco fecha_atn mensaje-div header-detalle">\
                </div>\
            </div > ';
        $('.header').html(header);
        DAO.execute("../_ctrl/ctrl.negocio.php", {
            ...
            function(r) {
                if (r.status == 202) {
                    ...
                    if (data.length > 0) {
                        ...
                        var fecha;
                        var minuto;
                        var fecha_atn = new Date(r.fecha);
                        var mensaje_atn = 'Tiempo excedido';
                        var hoy_atn = new Date();
                        if(fecha_atn.getDate() > hoy_atn.getDate() && fecha_atn.getFullYear() > hoy_atn.getFullYear()){
                          var fecha_atn = (fecha_atn.getDate()+1) + '/' + meses[fecha_atn.getMonth()] + '/' + fecha_atn.getFullYear();
                          mensaje_atn = 'Atender para el: '+fecha_atn;
                        }else if(fecha_atn.getDate() == hoy_atn.getDate() && fecha_atn.getFullYear() == hoy_atn.getFullYear()){
                          mensaje_atn = 'La cotización vence hoy';
                        }
                        $('.fecha_atn').html(mensaje_atn = '<span class="center-align">'+mensaje_atn + '</span>');
                        ...
                        data.forEach(function(element) {
                            ...
                            if (element.tipo_usuario == 1) {
                                ...
                                if (element.mensaje.indexOf('materialboxed') > -1) {
                                    ...
                                    buffer += element.mensaje.replace('offset-s2', '');
                                    buffer += '<span class="txt-gris-claro center-align hora">' + hora + ' </span>';
                                }else{
                    ...
                    $('.chat').html('<div class="txt-gris-claro margint-20 center-align">Aún no tienes ningún mensaje.</div>');
                }
            });
    },
    cotizacionDetalle: function() {
        ...
        Service.execute('../_ctrl/ctrl.negocio.php', {
            ...
            function(r) {
                if (r.status == 202) {
                    ...
                    var fecha_imprimir = (fecha.getDate()+1) + '/' + meses[fecha.getMonth()] + '/' + fecha.getFullYear();
        ...
        Service.execute("../_ctrl/ctrl.negocio.php", {
            ...
            if (r.status == 202) {
                ...
                if (usuario.movil) {
                    ...
                    $('.atelefono').attr('href', 'tel:+' + usuario.movil);
                }
                if (data.ubicacion) {
                    $('.mapa').html('<iframe width="100%" height="200" \
                        src="https://www.google.com/maps/embed/v1/place?q=' + data.ubicacion.lat + ',' + data.ubicacion.lng + '&key=AIzaSyBzHWQL9mUfvfV-iQzrrrhc_e-bS4PoiqU" frameborder="0"></iframe>');
